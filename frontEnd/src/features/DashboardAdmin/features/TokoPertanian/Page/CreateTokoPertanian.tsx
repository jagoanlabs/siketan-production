...
import {
  ...
  useCheckNip,
  useCreateProduct,
} from "@/hook/dashboard/tokoPertanian/useCreateTokoPertanian";
import PageMeta from "@/layouts/PageMeta";
import PageBreadcrumb from "@/components/Breadcrumb";

export const CreateTokoPertanian = () => {
  const navigate = useNavigate();
  const fileInputRef = useRef<HTMLInputElement>(null);

  // API Hooks
  const checkNikMutation = useCheckNik();
  const checkNipMutation = useCheckNip();
  ...
  const [userData, setUserData] = useState<
    NikCheckResponse["user"] | NipCheckResponse["user"] | null
  >(null);
  ...
  const validateForm = (): boolean => {
    const newErrors: CreateProductFormErrors = {};
  ...
  const getWilayahBinaan = () => {
    ...
    if (formData.profesiPenjual === "petani") {
      ...
      const penyuluhData = userData as NipCheckResponse["user"];
      const kecamatanBinaan =
        penyuluhData.kecamatanBinaanData
          ?.map((k) => k.kecamatan.nama)
          .join(", ") || penyuluhData.kecamatanBinaan;
      const desaBinaan =
        penyuluhData.desaBinaanData?.map((d) => d.desa.nama).join(", ") ||
  ...
  return (
    <div className="min-h-screen max-w-6xl container mx-auto py-6">
      <PageMeta
        description="Buat produk pertanian baru"
        title="Tambah Produk | Sistem Manajemen Pertanian"
      />
      <PageBreadcrumb
        items={[
          ...
          <div className="flex items-center gap-3 mb-6">
            ...
            <div>
              ...
              <p className="text-gray-600">
                Buat produk pertanian baru dari petani atau penyuluh
              </p>
            </div>
          </div>
        </div>
                ...
                <CardBody className="space-y-4">
                  ...
                  <div className="flex gap-2">
                    ...
                    <Button
                      ...
                      isLoading={isNikChecking}
                      variant="flat"
                      onPress={handleCheckNikNip}
                    >
                      {isNikChecking ? "Cek..." : "Cek Data"}
                    </Button>
                  </div>
