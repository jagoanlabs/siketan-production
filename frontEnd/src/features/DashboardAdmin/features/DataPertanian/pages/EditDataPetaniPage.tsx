...
export const EditDataPetaniPage: React.FC = () => {
  const navigate = useNavigate();
  const { id } = useParams<{ id: string }>();
  const petaniId = id ? parseInt(id) : undefined;
  ...
  const { data: petaniData, isLoading: isLoadingPetani } =
    useDataPetaniById(petaniId);
  ...
  useEffect(() => {
    if (petaniData) {
      setFormData({
        ...
        email: petaniData.email || petaniData.tbl_akun?.email || "",
        ...
        gapoktan: petaniData.kelompok?.gapoktan || "",
        namaKelompok: petaniData.kelompok?.namaKelompok || "",
      });
  ...
  if (isLoadingPetani) {
    return (
      <div className="min-h-screen container mx-auto max-w-6xl py-6">
        <PageMeta
          description="Edit data petani"
          title="Edit Data Petani | Sistem Manajemen Pertanian"
        />
        <PageBreadcrumb
          items={[
          ...
          <Card className="max-w-md mx-auto mt-20">
            <CardBody className="text-center py-10">
              ...
              <p className="text-gray-600 mb-4">
                Data petani dengan ID {id} tidak ditemukan.
              </p>
              <Button
                color="primary"
